...
% Sridhar: This is the bicycle model that should be in the introduction.

% The bicycle model defines a dynamical system.
% The \emph{state} of the system at time $t$
% is described by the triple $(x(t), y(t), \theta(t))$
% where $(x, y)$ is the coordinates of the rear wheel
% in some inertial frame, say the racing track, and
% $\theta$ is the angle of the direction of the car measured from the $x$-axis counter-clockwise.
% If the speed (magnitude of velocity) of the car is $v$,
% then
% \[
% \left\{
% \begin{array}{l}
...
% \right.
% \]
% where the dot operator is differentiation with respect to time.\footnote{In this paper, we measure distance in meters and angle in radians unless otherwise specified.}
...
% the lookahead point always lies on one of the line segments of the Voronoi diagram.
%
...
%there is a coupled evolution of the position and the orientation of the vehicle and the waypoint.
%
